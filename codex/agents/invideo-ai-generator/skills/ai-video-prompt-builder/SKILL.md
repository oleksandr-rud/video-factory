...
4. For image-to-video, describe motion and temporal progression more than static image details.
5. Add Prompt Guide notes for stable brand, character, camera, lighting, or voice rules that should persist across generations.
6. Add output fields for `positive_prompt`, `prompt_guides`, `settings`, and `reference_assets` in `codex/contracts/ai-video-generation-package.schema.json`.
