...
5. Create stable artifact paths under the project folder for the production run ledger, channel profile, producer criteria artifact, reference analysis, channel format, scenario, voiceover package, visual pack, candidates, AI video generation packages, Remotion clip packages, timeline sync plan, render packages, critique reports, and QA.
6. Run `producer-criteria-prompt` to create the first criteria artifact. Update scene-specific criteria after the scenario scene ids exist.
7. Define approval gates for paid APIs, licensed media, and generation jobs.
8. Build handoffs using `codex/contracts/agent-handoff.schema.json`.
9. Treat Visual Producer `handoff_recommendations[]` as routing input only. The Director decides whether to create downstream InVideo AI Generator or Remotion Clip Builder handoffs and names the target agent's local skills.
