...
5. Create or resolve the shared Remotion app contract using `codex/contracts/remotion-project.schema.json`; default to the repo `remotion/` app unless a project-specific app is justified.
6. Create stable repo-relative POSIX artifact paths under the project folder for the production run ledger, channel profile, producer criteria artifact, media asset manifest, reference analysis, channel format, scenario, voiceover package, visual pack, candidates, source media, AI video generation packages, Remotion clip packages, timeline sync plan, render packages, critique reports, review assets, and QA.
...
9. Build handoffs using `codex/contracts/agent-handoff.schema.json`; include project path, media asset manifest path, Remotion project contract path, channel profile path, and producer criteria path when available.
