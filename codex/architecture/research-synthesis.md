...
Local decision: scenario scenes are the identity source for scene-linked artifacts. The Director owns `scene-artifact-sync` reports so visual scene packs, candidates, AI generation packages, Remotion props, Remotion clip packages, and timeline sync can be checked for one-to-one coverage, stale fingerprints, orphaned scene ids, and route/template/media conflicts before render work proceeds.
...
6. Scene artifact sync is a Director-owned gate. Scenario scenes are the identity source, and every visual pack, AI package, Remotion props file, Remotion clip package, selected candidate, and timeline scene must prove current scene lineage before downstream work.
7. Scene-level candidate objects remain the cross-agent contract. A scene can compare Remotion-generated visuals, AI video generation, stock/provider clips, or user media with the same schema.
8. Planning and spending stay separated. Agents can prepare provider queries, prompts, cost estimates, and rankings without making paid calls until the Director gets approval.
9. Every important scene needs a primary and fallback route because search and generation are variable.
10. AI video generation delivery is tracked as a generation package, not just a prompt. The package should include positive prompt, negative constraints, prompt guides, model/settings, approval state, outputs, variants, and QA.
11. Remotion clip delivery is tracked separately from full render delivery. A clip package should include the composition id, component files, assets, props, preview/render commands, clip QA, and scene/props sync metadata.
12. Voiceover delivery is tracked separately from the scenario. A voiceover package should include inherited channel voice direction, selected provider voice, generation approval state, request payloads, audio paths, timestamp alignment, caption paths, pronunciation notes, and QA.
13. Timeline sync is tracked before full assembly. A timeline sync plan should align scene ids, scene indexes, scene pack ids, scene fingerprints, frame ranges, voiceover audio, captions, selected visual candidates, Remotion clip packages, props sync, transitions, and overlay safe areas.
14. Producer criteria are a first-class artifact, not a loose prompt. The Director creates them before production handoffs, passes the path downstream, and treats them as binding input for Video Critic review.
15. Final critique is tracked separately from technical render QA. A critique report should include sampled frame evidence, scene-by-scene gate results, artifact consistency findings, category scores, severity-ranked issues, limitations, and a prioritized revision plan.
16. Review loop state belongs in the production run ledger, not in every artifact. Each loop records the render candidate, critique report, failed gates, invalidated artifacts, rerun scope, routed actions, iteration count, next action, and stop reason.
17. Full render delivery is tracked as a render release candidate, not only a single MP4. The package should include the timeline, source clip packages, voiceover package, timeline sync plan, scene artifact sync report, render command, output file, subtitle artifacts, QA result, critique report, rights notes, and residual blockers.
18. Specialist-to-specialist work crosses through the Director. Visual Producer can recommend InVideo AI Generator or Remotion Clip Builder work, but it does not load those agents' skills or perform their feasibility checks locally.
