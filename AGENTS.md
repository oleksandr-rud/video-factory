...
When boundary checks, scene artifact sync, render QA, Video Critic, or user changes expose cross-artifact problems, the Director uses `codex/agents/director/references/artifact-problem-routing.md` to map the problem to the correct owner agent and create the repair handoff. The agent that detects a problem should not silently rewrite another agent's artifact.
